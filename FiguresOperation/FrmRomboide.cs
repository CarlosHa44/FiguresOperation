...
namespace FiguresOperation
{
    ...
    {
        private static FrmRomboide mInstance;
        ...
        {
            obj.InitializeData(txtBase, txtAltura, txtAngle, txtPerimeter, txtArea, picCanvas);
        }
        public static FrmRomboide GetInstance()
        {
            if (mInstance == null || mInstance.IsDisposed)
            {
                mInstance = new FrmRomboide();
            }
            return mInstance;
        }
        ...
        {
            obj.ResetTransform();
            obj.ReadData(txtBase,txtAltura,txtAngle);
